...
UPDATE_CHECKER_URL = 'https://packager.bilup.org/extras-version.json'
...
def create_installer(path):
  ...
  inno_config = f"""; Generated by Bilup Packager Extras v{VERSION}
; https://github.com/Bilup/packager-extras
...
def parse_zip(zip):
  ...
  if len(inner_folders) != 1:
    ...
    if 'project.json' in inner_folders:
      raise Exception('Zip appears to be a Scratch project. Please use packager.bilup.org to generate an  an "Electron Windows application" or "NW.js Windows application" application, then upload that zip into this program. (found project.json)')
...
class MainWindow(QtWidgets.QWidget):
  def __init__(self):
    ...
    self.label = QtWidgets.QLabel('Report bugs <a href="https://github.com/Bilup/packager-extras/issues">on GitHub</a>. Only run on files you trust.')
  ...
  def update_available(self, latest_version):
    ...
    self.label.setText(f'An update is available. <a href="https://github.com/Bilup/packager-extras/releases">Download v{escape_html(latest_version)} from GitHub releases</a>. ' + self.label.text())
...
def main():
  ...
  ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(u'org.bilup.packager.extras.' + VERSION)
